...
'''
Модуль с матемитаческим аппартом служит основой для создания,
решения  и  оценки всевозможных головоломок-судоку.  Аппарат
настроен на трехмерный случай, но легко может быть переделан
под n-мерный. Также возможно простое дополнение правил запол-
нения дополнительными (диагональные и др. правила).
'''

# Оценка уровня судоку относительно сложности
def getLevel(diff: int) -> str:
    if diff in range(0, 15):
        return 'very easy'
...
#  Математический аппарат для решения головоломки на основе частичного поиска с возвратом
...
def mix_puzzle(puzzle: list, times: int = 350) -> None:
...
def getMixedPuzzle(times: int = 350) -> list:
    puzzle = getExample()
